...
[NOTE]
With the exception of <<plasma:tube_cutting,tube cutting>> with an angular A or B axis, QtPlasmaC uses the LinuxCNC External Offsets feature for all Z axis motion, and for moving the X and/or Y axis for a consumable change or a cut recovery while paused.
...
If using an angular A or B axis for <<plasma:tube_cutting,tube cutting>> then Z axis motion is required in the G-code file. +
...
Tube cutting with an angular A or B axis is achieved with the following in the G-code file:
